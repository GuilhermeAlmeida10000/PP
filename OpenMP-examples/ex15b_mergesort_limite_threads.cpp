// Exemplo 15b: Merge sort recursivo, com limite de threads
// Compilar com: g++ ex15b_mergesort_limite_threads.cpp -fopenmp -o ex15b_mergesort_limite_threads -Wall
// Executar por linha de comando: ./ex15b_mergesort_limite_threads
...
void merge_sort_paralelo(int *a, int i, int f, int *tmp, int lim_threads)
{
	if (i < f) // Se não chegou na base da recursão
	{
		if (lim_threads == 1) // Se não possui mais threads para dividir trabalho
		{
			merge_sort_sequencial(a, i, f, tmp);
		}
		else // lim_threads > 1
		{
			int m = (i+f)/2;
...
int main(int argc, char** argv)
{
	if(argc != 2)
	{
		...
		printf("Uso: ./ex15b_mergesort_limite_threads <número_de_elementos>\n");
		exit(1);
	}
